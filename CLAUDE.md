...
- **Streaks** (`statsFor`): today gets grace if not yet filled; an available Wonder Wish (earned every `WISH_EVERY = 7` glowing nights) patches a missing night inside a chain. **`statsFor` is pure** — it reports the nights a wish *would* patch as `pendingWishes` so the streak reads right on every screen, and only `commitWishes()` (called from `renderStats`, i.e. the kid's own jar) writes the `wishspent` records and toasts. Don't reintroduce writes into `statsFor`: it runs for all four people on every profile render, which doesn't save. It also counts distinct *nights*, not entry records, so a cross-device duplicate can't inflate rewards.
...
- **No native dialogs** — never `alert`/`confirm`/`prompt`. Use `showModal`, `showInputModal`, `showPinModal`, `showTextModal`, and `toast(msg)` for transient feedback. `toast(msg, { label, onClick })` adds a button and holds 7s instead of 2.6s — that's how delete offers Undo (records are tombstones, so restoring is a flag flip).
- **Entry lists go through `attachEntryList`** (Thoughts and Family tabs) — it provides search across text/follow-ups/prompt/date plus 30-at-a-time paging, and redraws only the list so the search box keeps focus. Don't hand-roll a capped `slice()` list again; older entries become unreachable.
- **Anthropic API is strictly optional.** `askClaude` powers follow-up questions and the Wonder Report/Week of Wonder insights, but every AI path must silently no-op when `anthropic_key` is missing or the request fails — the app fully works without a key. Because failures are silent, a malformed request looks like "nothing happened": verify a real follow-up after changing the call. It targets `claude-sonnet-5` with `thinking: { type: 'disabled' }` — thinking is on by default on current models and shares the `max_tokens` budget, which would truncate a 60-token follow-up question to nothing.
